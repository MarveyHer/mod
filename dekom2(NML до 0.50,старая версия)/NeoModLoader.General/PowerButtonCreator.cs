...
public static class PowerButtonCreator
{
	...
	public static PowerButton CreateToggleButton(string pGodPowerId, Sprite pIcon, [CanBeNull] Transform pParent = null, Vector2 pLocalPosition = default(Vector2), bool pNoAutoSetToggleAction = false)
	{
		...
		PowerButton powerButton = ResourcesFinder.FindResource<PowerButton>("kings_and_leaders");
		bool activeSelf = ((Component)powerButton).gameObject.activeSelf;
		if (activeSelf)
		{
			((Component)powerButton).gameObject.SetActive(false);
		...
		static void toggleOption(string pPower)
		{
			...
			WorldTip.instance.showToolbarText(godPower2, pShowOnComputer: false);
	...
	public static PowersTab GetTab(string pId)
	{
		...
		Transform val = ((Component)CanvasMain.instance.canvas_ui).transform.Find("CanvasBottom/BottomElements/BottomElementsMover/CanvasScrollView/Scroll View/Viewport/Content/buttons/" + pId);
	...
	public static void AddButtonToTab(PowerButton button, PowersTab tab, Vector2 position)
	{
		...
		transform.localPosition = Vector2.op_Implicit(position);
		transform.localScale = Vector3.one;
