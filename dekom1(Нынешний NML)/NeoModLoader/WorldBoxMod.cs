...
public class WorldBoxMod : MonoBehaviour
{
	...
	private static void UnityExplorerFix()
	{
		...
		Harmony val = new Harmony("wbom.nml");
		MethodInfo methodInfo = AccessTools.Method(typeof(Assembly), "LoadFrom", new Type[1] { typeof(string) }, (Type[])null);
		MethodInfo methodInfo2 = AccessTools.Method(typeof(WorldBoxMod), "LoadFrom", (Type[])null, (Type[])null);
		ReversePatcher val2 = val.CreateReversePatcher((MethodBase)methodInfo, new HarmonyMethod(methodInfo2));
		val2.Patch((HarmonyReversePatchType)0);
	}
		...
		SmoothLoader.add(delegate
		{
			...
			foreach (ModDependencyNode mod2 in mod_nodes)
			{
				SmoothLoader.add(delegate
				{
					...
					{
						ResourcesPatch.LoadResourceFromFolder(Path.Combine(mod2.mod_decl.FolderPath, Paths.ModResourceFolderName), out var Builders);
						Builder.AddBuilders(Builders);
						ResourcesPatch.LoadResourceFromFolder(Path.Combine(mod2.mod_decl.FolderPath, Paths.NCMSAdditionModResourceFolderName), out var Builders2);
