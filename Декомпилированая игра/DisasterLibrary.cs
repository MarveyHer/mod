...
public class DisasterLibrary : AssetLibrary<DisasterAsset>
{
	...
	public DisasterAsset getRandomAssetFromPool()
	{
		...
		DisasterAsset tResult = tRandomPool.GetRandom();
		...
		if (tResult.min_world_population > World.world.units.Count)
		{
			return null;
		}
		return tResult;
	...
	public void spawnMadThought(DisasterAsset pAsset)
	{
		...
		if (tCity == null || tCity.getPopulationPeople() < 50 || tCity.getTile() == null)
		{
			return;
		}
		using ListPool<Actor> tMadUnits = new ListPool<Actor>(tCity.countUnits());
		foreach (Actor tUnit in tCity.units.LoopRandom())
		{
		...
		{
			tMadUnits[i].addTrait("madness");
		}
		WorldLog.logDisaster(pAsset, tCity.getTile(), null, tCity);
	}
	...
	public void spawnGreg(DisasterAsset pAsset)
	{
		...
		if (tCity != null)
		{
			Building tMine = tCity.getBuildingOfType("type_mine");
			if (tMine != null)
			{
				WorldTile tSpawnTile = tMine.current_tile;
				Actor tUnit = spawnDisasterUnit(pAsset, tSpawnTile);
				WorldLog.logDisaster(pAsset, tSpawnTile, tUnit.getName(), tCity, tUnit);
				spawnDisasterUnit(pAsset, tSpawnTile.region.tiles.GetRandom());
	...
	public void spawnNecromancer(DisasterAsset pAsset)
	{
		...
		{
			WorldTile tSpawnTile = tMine.current_tile;
			Actor tNecromancer = spawnDisasterUnit(pAsset, tSpawnTile);
			WorldLog.logDisaster(pAsset, tSpawnTile, tNecromancer.getName(), tCity, tNecromancer);
			int numSkeletons = Randy.randomInt(5, 25);
			for (int i = 0; i < numSkeletons; i++)
			{
				World.world.units.createNewUnit("skeleton", tSpawnTile.region.tiles.GetRandom(), pMiracleSpawn: false, 0f, null, null, pSpawnWithItems: true, pAdultAge: true);
			}
		}
	}
	...
	public void spawnEvilMage(DisasterAsset pAsset)
	{
		if (checkUnitSpawnLimits(pAsset))
		{
			TileIsland tIsland = World.world.islands_calculator.getRandomIslandGround();
			if (tIsland != null)
			{
				WorldTile tRandomTile = tIsland.getRandomTile();
				Actor tActor = spawnDisasterUnit(pAsset, tRandomTile);
	...
	public void spawnHeatwave(DisasterAsset pAsset)
	{
		...
		if (tHeatwave)
		{
			WorldLog.logDisaster(pAsset, tRandomIslandTile);
		}
	}

	public void spawnSmallEarthquake(DisasterAsset pAsset)
	{
		WorldTile tRandomTile = World.world.tiles_list.GetRandom();
		Earthquake.startQuake(tRandomTile, EarthquakeType.SmallDisaster);
		WorldLog.logDisaster(pAsset, tRandomTile);
	}
	...
	public void spawnDragon(DisasterAsset pAsset)
	{
		if (checkUnitSpawnLimits(pAsset))
		{
			TileZone tZone = ((!Randy.randomBool()) ? World.world.zone_calculator.getZone(World.world.zone_calculator.zones_total_x - 1, Randy.randomInt(0, World.world.zone_calculator.zones_total_y)) : World.world.zone_calculator.getZone(0, Randy.randomInt(0, World.world.zone_calculator.zones_total_y)));
			WorldTile tTile = tZone.centerTile;
			spawnDisasterUnit(pAsset, tTile);
			WorldLog.logDisaster(pAsset, tTile);
		}
	}
	...
	public void spawnBiomass(DisasterAsset pAsset)
	{
		...
		{
			Building tRandomBuilding = Randy.getRandom(item.buildings);
		...
		{
			WorldTile tTile = tBuildingTarget.tiles.GetRandom();
			if (spawnDisasterBuilding(pAsset, tTile) != null)
			{
				spawnDisasterUnit(pAsset, tTile);
				WorldLog.logDisaster(pAsset, tTile);
			}
	...
	public void gardenSurprise(DisasterAsset pAsset)
	{
		...
		foreach (City item in World.world.cities.list.LoopRandom())
		{
			Building tWindmill = item.getBuildingOfType("type_windmill");
	...
	{
		WorldTile tRandomTile = World.world.tiles_list.GetRandom();
		Meteorite.spawnMeteoriteDisaster(tRandomTile);
		WorldLog.logDisaster(pAsset, tRandomTile);
	}
	...
	public void simpleUnitAssetSpawnUsingIslands(DisasterAsset pAsset)
	{
		if (checkUnitSpawnLimits(pAsset))
		{
			...
			if (tIsland != null)
			{
				...
				spawnDisasterUnit(pAsset, tRandomIslandTile);
				WorldLog.logDisaster(pAsset, tRandomIslandTile);
			}
		}
	}
	...
	private Actor spawnDisasterUnit(DisasterAsset pAsset, WorldTile pTile)
	{
		...
		{
			tActor = World.world.units.createNewUnit(pAsset.spawn_asset_unit, pTile, pMiracleSpawn: false, 0f, null, null, pSpawnWithItems: true, pAdultAge: true, pGiveOwnerlessItems: true);
		}
		return tActor;
	}
